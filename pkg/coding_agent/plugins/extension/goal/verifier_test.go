...
func TestVerifierUnavailableNotesUnverifiedCompletion(t *testing.T) {
	e, api, tool := newVerifierExtension(t, map[string]any{"enabled": true})
	api.fork = func(context.Context, extension.ForkOptions) (string, error) {
		return "", errors.New("subagent exceeded max_turns=20")
	}
	active, _ := e.store.Current()
	e.mu.Lock()
	e.agentTurnInProgress = true
	...
	// Fork error → completion proceeds but is flagged as unverified.
	if _, err := tool.Execute(context.Background(), "tc", map[string]any{"status": "complete"}, nil); err != nil {
		t.Fatalf("update_goal: %v", err)
	}
	if g, _ := e.store.Current(); g.Status != StatusComplete {
		t.Fatalf("completion should proceed on verifier error, got %q", g.Status)
	}
	api.fireAgentEnd()

	found := false
	for _, notice := range api.notices {
		if strings.Contains(notice, "Completed without verification") {
			found = true
			break
		}
	}
	if !found {
		t.Fatalf("completion should be flagged unverified, got %#v", api.notices)
	}
}
