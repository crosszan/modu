...
func (t *workflowTool) Description() string {
	return strings.Join([]string{
		...
		"",
		"API:",
		"- meta({name, description, whenToUse?, phases?, exampleArgs?}) — name is kebab-case; description is one line; phase titles are unique; exampleArgs is a JSON-safe object. phases is [{title, detail?, model?}]. Call once, first.",
		...
		"",
		"Rules for reliable runs: always `await` agent/parallel/pipeline; end the script with `return <result>`; default to pipeline() and only use parallel() when you truly need all results at once; pass data between stages via the prompt string (use JSON.stringify for structured context); when meta.phases is present, every phase(title) and opts.phase value must use a declared title; set opts.phase inside parallel/pipeline stages so agents are grouped correctly.",
		"",
