...
- When idle, Enter emits `SubmitKindPrompt`. While the host is busy, Enter parks the message in the model's own pending queue instead of emitting anything: it renders in a region above the input, Backspace on an empty input takes the newest one back with its pastes and attachments, and Esc while interrupting discards the queue. Each `SetBusyUpdate{Busy: false}` releases the oldest queued message as `SubmitKindFollowUp` and only then appends it to the transcript, so nothing appears above output that predates it. Slash commands bypass the queue and dispatch immediately. Shift+Enter inserts a newline.
- Optional `Services.CheckSpelling` results decorate misspelled input ranges with a red curly underline. With the cursor on an issue, Tab calls `Services.SuggestSpelling`; slash-command and `@` file completion take precedence. Up/Down selects a candidate, Enter replaces the word, and Esc closes the popup. The host owns dictionary policy and performs both callbacks through Bubble Tea commands rather than during rendering.
...
- `Options.SlashCommands` or `Services.SlashCommands` supplies command suggestions. Tab completes a visible slash/file suggestion and never submits; Enter emits `SlashCommandIntent`.
